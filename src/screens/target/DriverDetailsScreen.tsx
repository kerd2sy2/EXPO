    ...
    >
      ...
      <View style={[styles.heroCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
        <View style={[styles.heroTopRow, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
          ...
          <View style={[styles.heroInfoCol, { alignItems: isRTL ? 'flex-end' : 'flex-start' }]}>
            <Text style={[styles.driverNameText, { color: colors.textPrimary, textAlign: isRTL ? 'right' : 'left' }]}>
              {driver.name}
            </Text>
            <View style={[styles.heroMetaRow, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
              {driver.phone ? (
                <View style={[styles.metaChip, { backgroundColor: colors.cardAlt }]}>
                  ...
                  <Text style={[styles.metaChipText, { color: colors.textSecondary }]}>{driver.phone}</Text>
                </View>
              ) : null}
              {driver.branch ? (
                <View style={[styles.metaChip, { backgroundColor: colors.accentLight }]}>
                  <Ionicons name="business" size={13} color={colors.accent} />
                  <Text style={[styles.metaChipText, { color: colors.accent, fontWeight: '800' }]}>فرع {driver.branch}</Text>
                </View>
              ) : null}
              <View style={[styles.metaChip, { backgroundColor: colors.primaryLight }]}>
                <Ionicons name="flag" size={13} color={colors.primary} />
                <Text style={[styles.metaChipText, { color: colors.primary, fontWeight: '800' }]}>التارچت: {dailyTarget}/يوم</Text>
              </View>
            </View>
          </View>
        </View>

        {/* Apps with Official Brand Logos & Identifiers tags */}
        {(Array.isArray(driver.apps) && driver.apps.length > 0) || (Array.isArray(driver.identifiers) && driver.identifiers.length > 0) ? (
          <View style={[styles.tagsContainer, { borderTopColor: colors.border }]}>
            {Array.isArray(driver.apps) && driver.apps.length > 0 && (
              <View style={[styles.tagRow, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
                ...
                <View style={[styles.tagValuesWrap, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
                  {driver.apps.map((app, idx) => {
                    ...
                    return (
                      ...
                      >
                        {meta.logo ? (
                          <View style={[styles.appLogoMiniBox, { backgroundColor: meta.avatarBg }]}>
                            <Image
                              source={meta.logo}
                              style={styles.appLogoMiniImg}
                              resizeMode={meta.resizeMode}
                            />
                          </View>
                        ) : (
                          <Ionicons name="apps-outline" size={12} color={meta.text} />
                        )}
                        <Text style={[styles.appBrandPillText, { color: meta.text }]}>{app}</Text>
                      </View>
                    );
                  })}
                </View>
            ...
            {Array.isArray(driver.identifiers) && driver.identifiers.length > 0 && (
              <View style={[styles.tagRow, { flexDirection: isRTL ? 'row-reverse' : 'row', marginTop: 8 }]}>
                <Text style={[styles.tagLabel, { color: colors.textSecondary }]}>المعرفات المسجلة:</Text>
                <View style={[styles.tagValuesWrap, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
                  {driver.identifiers.map((ident, idx) => (
                    <View key={idx} style={[styles.identPillHero, { backgroundColor: colors.cardAlt, borderColor: colors.border }]}>
                      <Ionicons name="key-outline" size={12} color={colors.primary} />
                      <Text style={[styles.identPillHeroText, { color: colors.textPrimary }]}>{ident}</Text>
                    </View>
                  ))}
                </View>
              </View>
