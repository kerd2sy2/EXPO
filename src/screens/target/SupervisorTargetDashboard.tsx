    ...
    try {
      ...
      const [sumData, identsData, driversData, alertsData] = await Promise.all([
        targetApi.getDashboard(dateFilter.month, branchFilter, dateFilter.startDate, dateFilter.endDate).catch((err) => {
          console.log('Notice loading supervisor target dashboard:', err?.message || err);
          return null;
        }),
        targetApi.listIdentifiers({
          ...
          console.log('Notice loading supervisor target identifiers:', err?.message || err);
          return null;
        }),
                    ...
                    >
                      <View style={[styles.itemTopRow, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
                        <View style={{ alignItems: isRTL ? 'flex-end' : 'flex-start', flex: 1 }}>
                          <Text style={[styles.itemName, { color: colors.textPrimary }]}>
                            {alert.identifier_name || 'بدون اسم'}
                          </Text>
                          <View style={[styles.alertDriverRow, { flexDirection: isRTL ? 'row-reverse' : 'row', alignItems: 'center', gap: 4, marginTop: 4 }]}>
                            <Ionicons name="person-outline" size={13} color={colors.primary} />
                            <Text style={[styles.alertDriverText, { color: colors.textSecondary }]}>
                              المندوب المسؤول اليوم: <Text style={{ color: colors.textPrimary, fontWeight: '700' }}>{getAssignedDriverName(alert)}</Text>
                            </Text>
                          </View>
                          <Text style={[styles.itemCode, { color: colors.textSecondary, marginTop: 3 }]}>
